...
1. BaseModel<T> : Inherit from QAbstractItemModel. Handle drop(T) events, edit with caching, expose getValue function etc. Owns the `flags()` dispatch: the `QModelIndex` override is `final` and resolves the element before delegating to the virtual `flags(const T&, const QModelIndex&)` customization point (default `Qt::ItemIsSelectable | Qt::ItemIsEnabled`). The flags returned for the invalid/root index come from a settable `rootFlags()` / `setRootFlags()` attribute (default `Qt::NoItemFlags`).
