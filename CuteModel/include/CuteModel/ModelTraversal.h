                  ...
                  const QModelIndex &parent = QModelIndex())
{
    ...
    for (int row = 0; row < rows; ++row) {
        ...
        if (childParent.isValid() && model.hasChildren(childParent))
            forEachIndex<T>(accessor, model, visit, columns, childParent);
    }
}
...
void forEachIndex(const AbstractSourceModel<T> &model, Visitor &&visit,
                  ...
                  const QModelIndex &parent = QModelIndex())
{
    forEachIndex<T>(static_cast<const ValueModelAccessor<T> &>(model),
                    static_cast<const QAbstractItemModel &>(model),
                    std::forward<Visitor>(visit), columns, parent);
}
